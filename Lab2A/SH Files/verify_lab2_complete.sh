...
test_alb_direct_access() {
  ...
  if [ "$ALB_DNS" = "UNKNOWN" ] || [ -z "$ALB_DNS" ]; then
    log_failure "Could not find ALB DNS name (jarvis-alb01)"
    return 1
  fi
...
test_cloudfront_access() {
  ...
  DOMAIN=$(aws ssm get-parameter \
    ...
    --output text 2>/dev/null || echo "jarvisStarkDEVOPS.com")
...
test_dns_configuration() {
  ...
  ALB_IP=$(aws elbv2 describe-load-balancers \
    --query 'LoadBalancers[?LoadBalancerName==`jarvis-alb01`].LoadBalancerAddresses[0].IpAddress' \
...
test_origin_header() {
  ...
  HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" --connect-timeout 5 \
    -H "X-jarvis-Growl: fake-secret-value" \
    https://"$ALB_DNS" 2>/dev/null || echo "000")
