################################################################################
# jarvis 1b — Incident Response Infrastructure
# 
...
resource "aws_cloudwatch_log_group" "jarvis_rds_app_logs" {
...
resource "aws_sns_topic" "jarvis_db_incidents" {
  name = "jarvis-db-incidents"
...
resource "aws_sns_topic_policy" "jarvis_db_incidents_policy" {
  arn = aws_sns_topic.jarvis_db_incidents.arn
    ...
    Statement = [
      {
        ...
        Action   = "SNS:Publish"
        Resource = aws_sns_topic.jarvis_db_incidents.arn
      }
    ]
  })
}
...
resource "aws_cloudwatch_log_metric_filter" "db_error_filter" {
  name           = "DBErrorFilter"
  log_group_name = aws_cloudwatch_log_group.jarvis_rds_app_logs.name
  pattern        = "[ERROR]"
...
resource "aws_iam_role_policy" "ec2_cloudwatch_logs" {
  ...
  role = aws_iam_role.jarvis_ec2_role01.id
    ...
    Statement = [
      {
        ...
        Resource = "${aws_cloudwatch_log_group.jarvis_rds_app_logs.arn}:*"
      },
      {
        Effect = "Allow"
        Action = [
        ...
        Condition = {
          StringEquals = {
            "cloudwatch:namespace" = "jarvis/RDSApp"
          }
        }
      }
    ]
...
output "jarvis_db_incidents_topic_arn" {
  description = "SNS Topic ARN for database incident notifications"
  value       = aws_sns_topic.jarvis_db_incidents.arn
}

output "jarvis_db_incidents_topic_name" {
  description = "SNS Topic name for database incident notifications"
  value       = aws_sns_topic.jarvis_db_incidents.name
}

output "log_group_name" {
  description = "CloudWatch Log Group for application logging"
  value       = aws_cloudwatch_log_group.jarvis_rds_app_logs.name
}
