...
resource "aws_internet_gateway" "jarvis_igw01" {
...
resource "aws_subnet" "jarvis_public_subnets" {
...
resource "aws_subnet" "jarvis_private_subnets" {
...
resource "aws_nat_gateway" "jarvis_nat01" {
  allocation_id = aws_eip._nat_eip01.id
  subnet_id     = aws_subnet.jarvis_public_subnets[0].id # NAT in a public subnet

  tags = {
    Name = "${local.name_prefix}-nat01"
  }

  depends_on = [aws_internet_gateway.jarvis_igw01]
}
...
resource "aws_route_table" "jarvis_public_rt01" {
  vpc_id = aws_vpc.jarvis_vpc01.id
...
resource "aws_route" "jarvis_public_default_route" {
  route_table_id         = aws_route_table.jarvis_public_rt01.id
  destination_cidr_block = "0.0.0.0/0"
  gateway_id             = aws_internet_gateway.jarvis_igw01.id
}

# Explanation: Attach public subnets to the “public lanes.”
resource "aws_route_table_association" "jarvis_public_rta" {
  count          = length(aws_subnet.jarvis_public_subnets)
  subnet_id      = aws_subnet.jarvis_public_subnets[count.index].id
  route_table_id = aws_route_table.jarvis_public_rt01.id
}
...
resource "aws_route" "jarvis_private_default_route" {
...
resource "aws_route_table_association" "jarvis_private_rta" {
  count          = length(aws_subnet.jarvis_private_subnets)
  subnet_id      = aws_subnet.jarvis_private_subnets[count.index].id
  route_table_id = aws_route_table._private_rt01.id
}
...
resource "aws_security_group" "jarvis_ec2_sg01" {
...
resource "aws_security_group" "jarvis_ec2_sg02" {
...
resource "aws_vpc_security_group_ingress_rule" "jarvis_ec2_sg_ingress_http" {
  ip_protocol       = local.tcp_protocol
  security_group_id = aws_security_group.jarvis_ec2_sg01.id
...
resource "aws_vpc_security_group_egress_rule" "jarvis_ec2_sg_egress_db" {
...
resource "aws_security_group" "jarvis_rds_sg01" {
...
# TODO: student adds inbound MySQL 3306 from aws_security_group.jarvis_ec2_sg01.id

resource "aws_vpc_security_group_ingress_rule" "jarvis_rds_sg_ingress_mysql" {
  ip_protocol                  = local.tcp_protocol
  security_group_id            = aws_security_group.jarvis_rds_sg01.id
  from_port                    = local.db_port
  to_port                      = local.db_port
  referenced_security_group_id = aws_security_group.jarvis_ec2_sg01.id #allow traffic ONLY from specified SG
}
...
resource "aws_db_subnet_group" "jarvis_rds_subnet_group01" {
  name       = "${local.name_prefix}-rds-subnet-group01-v2"
  subnet_ids = aws_subnet.jarvis_private_subnets[*].id
...
data "aws_db_instance" "jarvis_existing" {
  db_instance_identifier = "${local.name_prefix}-rds01"
}
...
resource "aws_db_instance" "jarvis_rds01" {
  ...
  # Move the DB into the new VPC by switching to the new subnet group + SG.
  db_subnet_group_name   = aws_db_subnet_group.jarvis_rds_subnet_group01.name
  vpc_security_group_ids = [aws_security_group.jarvis_rds_sg01.id]
  ...
  depends_on = [aws_db_subnet_group.jarvis_rds_subnet_group01, aws_security_group.jarvis_rds_sg01]
}
...
resource "aws_iam_role" "jarvis_ec2_role01" {
...
resource "aws_iam_role" "jarvis_ec2_role02" {
...
resource "aws_iam_role_policy_attachment" "jarvis_ec2_ssm_attach" {
  role       = aws_iam_role.jarvis_ec2_role01.name
  policy_arn = "arn:aws:iam::aws:policy/AmazonSSMManagedInstanceCore"
}
# resource "aws_iam_role_policy_attachment" "jarvis_ec2_ssm_attach02" {
#   role       = aws_iam_role.jarvis_ec2_role02.name
#   policy_arn = "arn:aws:iam::aws:policy/AmazonSSMManagedInstanceCore"
# }

# Explanation: EC2 must read secrets/params during recovery—give it access (students should scope it down).
resource "aws_iam_role_policy_attachment" "jarvis_ec2_secrets_attach" {
  role       = aws_iam_role.jarvis_ec2_role01.name
  policy_arn = aws_iam_policy.jarvis_secrets_policy.arn
}

# Explanation: CloudWatch logs are the “ship’s black box”—you need them when things explode.
resource "aws_iam_role_policy_attachment" "jarvis_ec2_cw_attach" {
  role       = aws_iam_role.jarvis_ec2_role01.name
  policy_arn = "arn:aws:iam::aws:policy/CloudWatchAgentServerPolicy"
}

# Explanation: Instance profile is the harness that straps the role onto the EC2 like bandolier ammo.
resource "aws_iam_instance_profile" "jarvis_instance_profile01" {
...
resource "aws_iam_instance_profile" "jarvis_instance_profile02" {
  name = "${local.name_prefix}-instance-profile02"
  role = aws_iam_role.jarvis_ec2_role02.name
}
resource "aws_iam_policy" "jarvis_secrets_policy" {
  name        = "secrets_policy"
    ...
    "Statement" : [
      {
        ...
        "Resource" : "${aws_secretsmanager_secret.jarvis_db_secret01.arn}*"
      }
    ]
  })
...
resource "aws_instance" "jarvis_bastion_host_ec2_02" {
  ami                    = "ami-0030e4319cbf4dbf2" # Ubuntu 22.04 LTS in us-east-1
  instance_type          = var.ec2_instance_type
  subnet_id              = aws_subnet.jarvis_public_subnets[0].id
  vpc_security_group_ids = [aws_security_group.jarvis_ec2_sg02.id]
  iam_instance_profile   = aws_iam_instance_profile.jarvis_instance_profile01.name
...
resource "aws_instance" "jarvis_ec2_01" {
  ami                         = "ami-0030e4319cbf4dbf2" # Ubuntu 22.04 LTS in us-east-1
  instance_type               = var.ec2_instance_type
  subnet_id                   = aws_subnet.jarvis_public_subnets[0].id
  vpc_security_group_ids      = [aws_security_group.jarvis_ec2_sg01.id]
  iam_instance_profile        = aws_iam_instance_profile.jarvis_instance_profile01.name
  user_data_replace_on_change = true
  associate_public_ip_address = true
  ...
  # TODO: student supplies user_data to install app + CW agent + configure log shipping
  user_data  = file("${path.module}/1a_user_data.sh")
  depends_on = [aws_db_instance.jarvis_rds01]

  tags = {
    Name = "${local.name_prefix}-ec2_01"
  }
}
resource "aws_instance" "jarvis_ec2_03" {
  ami                    = "ami-0030e4319cbf4dbf2" # Ubuntu 22.04 LTS in us-east-1
  instance_type          = var.ec2_instance_type
  subnet_id              = aws_subnet.jarvis_private_subnets[0].id
  vpc_security_group_ids = [aws_security_group.jarvis_ec2_sg02.id]
  iam_instance_profile   = aws_iam_instance_profile.jarvis_instance_profile02.name
  #user_data_replace_on_change = true
...
# Explanation: Parameter Store is jarvis’s map—endpoints and config live here for fast recovery.
resource "aws_ssm_parameter" "jarvis_db_endpoint_param" {
...
resource "aws_ssm_parameter" "jarvis_db_port_param" {
  name      = "/lab/db/port"
  type      = "String"
  value     = tostring(aws_db_instance.jarvis_rds01.port)
  overwrite = true # Allow overwrite if parameter already exists
...
resource "aws_ssm_parameter" "jarvis_db_name_param" {
  name      = "/lab/db/name"
  type      = "String"
  value     = var.db_name
  overwrite = true # Allow overwrite if parameter already exists
...
# Explanation: Secrets Manager is jarvis’s locked holster—credentials go here, not in code.
#Recovery_window_in_days forces deletion of secrets and allows re-deployment of secret without constantly changing name

resource "aws_secretsmanager_secret" "jarvis_db_secret01" {
...
resource "aws_secretsmanager_secret_version" "jarvis_db_secret_version01" {
  secret_id = aws_secretsmanager_secret.jarvis_db_secret01.id

  secret_string = jsonencode({
    username = var.db_username
    password = var.db_password
    host     = aws_db_instance.jarvis_rds01.address
    port     = aws_db_instance.jarvis_rds01.port
    dbname   = var.db_name
  })
}
...
# resource "aws_cloudwatch_log_group" "jarvis_log_group01" {
...
#   }
#
#   depends_on = [aws_vpc.jarvis_vpc01]
# }
...
resource "aws_cloudwatch_metric_alarm" "jarvis_db_alarm01" {
  ...
  alarm_actions = [aws_sns_topic.jarvis_sns_topic01.arn]
...
resource "aws_sns_topic" "jarvis_sns_topic01" {
...
resource "aws_sns_topic_subscription" "jarvis_sns_sub01" {
  topic_arn = aws_sns_topic.jarvis_sns_topic01.arn
  protocol  = "email"
