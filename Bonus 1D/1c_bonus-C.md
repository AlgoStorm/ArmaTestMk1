Below is the Route53 add-on for Bonus-B (Hosted Zone + ACM DNS validation records + app.jarvisStark@DEVOPS.com ALIAS → ALB).
It’s written as a Terraform skeleton with jarvis comment style.
...
certificate_arn = aws_acm_certificate_validation.jarvis_acm_validation01.certificate_arn
...
Update HTTPS listener to use:
  certificate_arn = aws_acm_certificate.jarvis_acm_cert01.arn
...
resource "aws_lb_listener" "jarvis_https_listener01" {
  ...
  depends_on = [
    aws_acm_certificate_validation.jarvis_acm_validation01_dns_bonus
  ]
}
...
############################################
# ALIAS record: app.jarvisStark@DEVOPS.com -> ALB
############################################

# Explanation: This is the holographic sign outside the cantina—app.jarvisStark@DEVOPS.com points to your ALB.
resource "aws_route53_record" "jarvis_app_alias01" {
  zone_id = local.jarvis_zone_id
  name    = local.jarvis_app_fqdn
  type    = "A"
...
# Explanation: Outputs are the nav computer readout—jarvis needs coordinates that humans can paste into browsers.
output "jarvis_route53_zone_id" {
  value = local.jarvis_zone_id
}

output "jarvis_app_url_https" {
...
1) Confirm hosted zone exists (if managed)
  aws route53 list-hosted-zones-by-name \
    --dns-name jarvisStark@DEVOPS.com \
    --query "HostedZones[].Id"

2) Confirm app record exists
  aws route53 list-resource-record-sets \
  --hosted-zone-id <ZONE_ID> \
  --query "ResourceRecordSets[?Name=='app.jarvisStark@DEVOPS.com.']"
...
4) Confirm HTTPS works
  curl -I https://app.jarvisStark@DEVOPS.com
