Ready to Suffer? —here’s the next realism bump for Lab 1C-Bonus-D:
  1) Zone apex (jarvis-roar.com) ALIAS → ALB
...
In bonus_b.tf, inside resource "aws_lb" "jarvis_alb01" { ... } add:

  # Explanation: jarvis keeps flight logs—ALB access logs go to S3 for audits and incident response.
  access_logs {
...
output "jarvis_apex_url_https" {
  value = "https://${var.domain_name}"
}

# Explanation: Log bucket name is where the footprints live—useful when hunting 5xx or WAF blocks.
output "jarvis_alb_logs_bucket_name" {
  value = var.enable_alb_access_logs ? aws_s3_bucket.jarvis_alb_logs_bucket01[0].bucket : null
}
...
1) Verify apex record exists
  aws route53 list-resource-record-sets \
    ...
    --query "ResourceRecordSets[?Name=='jarvis-roar.com.']"
